...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    float invW=1.0/max(iResolution.x,1.0);
    float sc=512.0/iResolution.x*.4;
    
    vec2 uv = (frag-C)*sc;
    
    // Normalized offset from -1 to 1 based on props
    vec2 nOff = vec2(uBeamXFrac * 2.0 - 1.0, uBeamYFrac * 2.0 - 1.0);
    vec2 screen_off = nOff * (iResolution.xy * 0.5 * sc);
    
    vec2 uvc = uv - screen_off;
    ...
    fuv+=uFlowTime*uFogFallSpeed*dir;
    vec2 prp=vec2(-dir.y,dir.x);
    fuv+=prp*(0.08*sin(dot(uvc,prp)*0.08+uFlowTime*0.9));
...
const LaserFlow = ({
    ...
    useEffect(() => {
        ...
        const uniforms = {
            ...
            uFade: { value: hasFadedRef.current ? 1.0 : 0.0 }
        };
        ...
        let prevTime = 0;
        let fade = hasFadedRef.current ? 1.0 : 0.0;
        ...
        const animate = () => {
            ...
            if (!hasFadedRef.current) {
                const fadeDur = 1.0;
                fade = Math.min(1.0, fade + cdt / fadeDur);
                uniforms.uFade.value = fade;
                if (fade >= 1.0) hasFadedRef.current = true;
            }
